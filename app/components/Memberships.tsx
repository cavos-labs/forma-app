...
interface MembershipsProps {
  onCreateUser: () => void;
  onEditUser: (membership: MembershipData) => void;
...
const Memberships = forwardRef<MembershipsRef, MembershipsProps>(
  ({ onCreateUser, onEditUser, onViewReceipt }, ref) => {
    ...
    const loadMemberships = useCallback(async () => {
      ...
      try {
        ...
        if (response.success && response.memberships) {
          ...
          const transformedMemberships: MembershipData[] =
    ...
    const statusCounts = {
      ...
      pending_payment: memberships.filter((m) => m.status === "pending_payment")
        .length,
    ...
    return (
      <div className="p-3 sm:p-6">
        ...
        <div className="flex overflow-x-auto gap-2 mb-4 pb-2 -mx-3 px-3">
          {Object.entries(statusCounts).map(([status, count]) => (
            <button
              key={status}
              onClick={() =>
                setStatusFilter(status as MembershipStatus | "all")
              }
            ...
            >
              {status === "all"
                ...
                ? t("Activas", "Active")
                : status === "pending_payment"
                ? t("Pendiente Pago", "Pending Payment")
                : status === "expired"
                ? t("Vencidas", "Expired")
                : status === "inactive"
                ? t("Inactivas", "Inactive")
                : t("Canceladas", "Cancelled")}{" "}
              ({count})
            </button>
          ))}
        </div>
          ...
          <div className="flex justify-center items-center py-12">
            <div className="flex items-center space-x-2">
              ...
              <span style={{ color: colors.foreground }}>
                {t("Cargando membresías...", "Loading memberships...")}
              </span>
            </div>
          </div>
        )}
                        ...
                        <span
                          ...
                          style={{
                            backgroundColor: getPaymentStatusColor(
                              membership.latest_payment.status
                            ),
                          }}
                        >
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
                      ...
                      <button
                        ...
                        className="text-xs px-2.5 sm:px-3 py-1.5 rounded-lg font-bold transition-all duration-200 hover:scale-105 hover:shadow-md active:scale-95 flex-shrink-0"
                        style={{
                        ...
                        title={t("Editar cliente", "Edit client")}
                      >
                        {t("Editar Cliente", "Edit Client")}
                      </button>

                      {/* Receipt Button or Status */}
                      {membership.latest_payment?.payment_proof_url &&
                      !membership.latest_payment.payment_proof_url.includes(
                        "pending-upload"
                      ) ? (
                        <button
                          onClick={() => {
                            onViewReceipt({
                              ...
                              paymentInfo: {
                                ...
                                reference:
                                  membership.latest_payment!.sinpe_reference ||
                                  undefined,
                                phone:
                                  membership.latest_payment!.sinpe_phone ||
                                  undefined,
                              },
                            });
                          }}
                      ...
                      {membership.status !== "active" && (
                        <button
                          ...
                          className="text-xs px-2.5 sm:px-3 py-1.5 rounded-lg font-bold transition-all duration-200 hover:scale-105 hover:shadow-md active:scale-95 disabled:opacity-50 flex-shrink-0"
                          style={{
                    ...
                    {/* Payment Action Buttons - Only for pending payments */}
                    {membership.latest_payment?.status === "pending" && (
                      <div className="flex flex-wrap gap-1.5 sm:gap-2">
                        <button
                          onClick={() =>
                            updatePaymentStatus(
                              membership.latest_payment!.id,
                              "approved"
                            )
                          }
                          className="px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 hover:scale-105 hover:shadow-md active:scale-95 flex-1 min-w-[70px] sm:min-w-[80px]"
                          style={{
              ...
              <div className="text-center py-12">
                <p style={{ color: colors.foreground, opacity: 0.6 }}>
                  {t(
                    ...
                    "No memberships found with the applied filters."
                  )}
                </p>
              </div>
            )}
        ...
        {/* Edit User Modal */}
        <EditUserModal
          isOpen={false} // This modal is now managed by the parent
          onClose={() => {}}
          membership={null}
          onSave={() => {}}
          onSuccess={() => {}}
                ...
                <button
                  onClick={() =>
                    updatePaymentStatus(
                      selectedPayment.id,
                      "rejected",
                      selectedPayment.rejection_reason || undefined
                    )
                  }
